  ...
  <article class="group">
    <RouterLink :to="{
      ...
      <picture class=" block rounded-t-lg overflow-hidden">
        <img class="transition-all duration-300 ease-in-out group-hover:scale-110" :src="imgCardPath" :alt="imgAlt" />
      </picture>
    </RouterLink>
    <div class="relative bg-white px-6 py-8 border border-indigo-200 border-t-0 rounded-b-lg space-y-4">
      ...
      <h3 class="text-2xl font-semibold">
        <RouterLink class="text-black" to="/">{{ title }}</RouterLink>
      </h3>
      <button class="absolute right-6 top-8 !mt-0 border border-indigo-200 rounded-full p-2" @click="like = !like">
        <IconLove class="stroke-indigo-500 relative top-[1px]" :class="{ 'fill-indigo-500': !like }" />
      </button>
      ...
      <ul class="flex justify-between text-sm *:flex *:gap-2">
        <li>
          <IconBed class="size-5 stroke-indigo-500" /> <span>{{ beds }} beds</span>
        </li>
        <li>
